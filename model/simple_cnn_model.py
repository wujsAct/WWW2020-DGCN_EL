...
class SimpleCNNLocalEntLinkModel(Model):
  def __init__(self,args):
    super(SimpleCNNLocalEntLinkModel, self).__init__()
  ...
  def _local_score(self):
    ...
    if self.args.feature=='S0':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S0_score[feat_dim]],-1)
    elif  self.args.feature=='S1':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S1_score[feat_dim]
                            ],-1)
    elif self.args.feature=='S2':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S2_score[feat_dim]
                            ],-1)
    elif self.args.feature=='S01':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S0_score[feat_dim],
                            self.S1_score[feat_dim]
                            ],-1)
    elif self.args.feature=='S02':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S0_score[feat_dim],
                            self.S2_score[feat_dim]
                            ],-1)
    elif self.args.feature=='S12':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S1_score[feat_dim],
                            self.S2_score[feat_dim]
                            ],-1)
    elif self.args.feature=='S012':
      self.local_p_e_ctx_score = tf.concat([
                            self.p_e_m,
                            self.S0_score[feat_dim],
                            self.S1_score[feat_dim],
